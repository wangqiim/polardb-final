...
static Status Get(int32_t select_column,
  ...
  if(is_full_recovery) {
    if (where_column == Id) {
      ...
      // auto it = pk_uk.find(*(int64_t *)(column_key));
      // if (it != pk_uk.end()) {
      //   // _mm_prefetch(it -> second.data, _MM_HINT_T0);
      //   com::xmemcopy<64>::copy(res, it -> second.data);
      //   com::xmemcopy<64>::copy(res + 64, it -> second.data + 64);
      //   ans = 1;
      // }
      // return ans;
      char *data = pk__uk.get(*(uint64_t *)column_key);
      com::xmemcopy<128>::copy(res, data);
      return 1;
    }
    ...
    if (where_column == Userid) {
      ...
      // auto it = uk_name.find(*(uint64_t *)column_key);
      // com::xmemcopy<64>::copy(res, it -> second.data);
      // com::xmemcopy<64>::copy(res + 64, it -> second.data + 64);
      // // memcpy(res, it -> second.data, 128UL);
      // return 1;
    ...
    if (where_column ==  Salary) {
      // int ans = 0;
      // // read_salay_id_count++;
      // auto it = sk_pk.find(*(uint64_t *)(column_key));
      // if (it != sk_pk.end()) {
      //   ans = 1;
      //   com::xmemcopy<8>::copy(res, &it -> second);
      // }
      // return ans;
      uint64_t result = sk__pk.get(*(uint64_t *)(column_key));
      com::xmemcopy<8>::copy(res, &result);
      return 1;
    }
  }
...
static Status WriteMemory(const char *tuple, size_t len, uint64_t recovery_count) {
  if(recovery_count < 100000) {
    memcpy(table + recovery_count, tuple, 272UL);
    ...
    if (it != sk.end()) {
        ...
        sk.insert(std::pair<uint64_t, std::vector<uint64_t>>(user -> salary, {recovery_count}));
    }
  }
  User *user = (User *)tuple;
