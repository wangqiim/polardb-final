...
static MyMultiMap<uint64_t, uint64_t> sk[SK_HASH_MAP_SHARD];

// static emhash7::HashMap<uint64_t, uint32_t> pk[PK_HASH_MAP_SHARD];
static emhash7::HashMap<UserId, uint64_t, UserIdHash> uk[UK_HASH_MAP_SHARD];
...
//    uk[uk_shard].insert(std::pair<UserId, Value>(UserId(uk_hash), Value(pos, id, salary)));
  uk[uk_shard].insert(std::pair<UserId, uint64_t>(UserId(uk_hash), id));
  // 3. insert sk index
  sk[sk_shard].insert(std::pair<uint64_t, uint64_t>(salary, id));
...
static std::vector<uint64_t> getPosFromKey(int32_t where_column, const void *column_key, bool is_local) {
