...
hashtable_create(int32_t size) {
    ...
    ht->size = size;
    ht->len = 0;
    ht->slots = (struct bucket **)allocfn(ht->size * sizeof(int64_t));
...
int
hashtable_put(struct hashtable *ht, const char *key, uint64_t val) {
    uint32_t hashid = hashfn(key, HASH_ID);
    struct bucket *bkt = (struct bucket *)allocfn(sizeof(*bkt));
